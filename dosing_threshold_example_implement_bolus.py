...
def prepare_bg(df, current_time):
    ...
    df_axis = dict(
            domain=[0.5, 1],
            range=[0, 250],
            tickvals=[-100, 54, 70, 140, 180, 250],
            fixedrange=True,
            hoverformat=".0f",
            zeroline=False,
            showgrid=True,
...
#    target_at_eval_df = df[df["datetime"] == current_time]
    ...
    for i in range(0, len(target)):
        ...
        tmp_trace = go.Scatter(
            name="Correction Range (100-120 mg/dL)",
    ...
    correction_target_trace = go.Scatter(
        name="Dosing Decision Safety Threshold",
    ...
    return df, target_traces, correction_details
...
def prepare_loop_prediction(predicted_bg_dates, predicted_bg_values):
    bg_prediction_trace = go.Scattergl(
        name="Forecasted Glucose (WITH rec. bolus dose)",
...
custom_scenario_files = [
    ...
    "dosing-threshold-example-1-implement-bolus.csv"
]
path = os.path.join(".", "example_files")
table_path_name = os.path.join(path, custom_scenario_files[5])
...
loop_output = loop_algorithm_output
...
# correction range
target_range, target_traces, correction_details = (
    prepare_target_range(
        df_target_range, continguous_ts, current_time, loop_output
    )
)
correction_details.to_csv("dosing-threshold-example-correction-dose.csv")
# suspend threshold
suspend_threshold = inputs.get("settings_dictionary").get("suspend_threshold")
suspend_trace = prepare_suspend(suspend_threshold, current_time)
...
# traces.extend(basal_delivered_traces)
# traces.extend([loop_basal_trace, loop_bolus_trace])
# traces.extend([loop_bolus_trace])
plotly_fig = go.Figure(data=traces, layout=fig_layout)
